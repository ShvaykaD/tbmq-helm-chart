...
> **📜 Trademarks:** This software listing is packaged by ThingsBoard Team. 
...
| installation.argocd          | Enables ArgoCD-specific Helm annotations for managing TBMQ deployments with ArgoCD. When set to true, the chart applies the following ArgoCD hooks: <br/> argocd.argoproj.io/hook: Sync  – Ensures that the Helm release is treated as a sync hook. <br/> argocd.argoproj.io/hook-delete-policy: HookSucceeded – Automatically removes the hook resources once the sync operation completes successfully. | false                       |

## TBMQ-Specific Parameters

This section describes the configuration options for the **ThingsBoard MQTT Broker (TBMQ)** and its **Integration Executor** component.
...
| tbmq.statefulSet.annotations                | Custom annotations for StatefulSet.                                                                                                                                                                                                                                | { }                                     |
| **Ports configuration**                     |                                                                                                                                                                                                                                                                    |                                         |
...
| tbmq.annotations                            | Allows users to define custom annotations for the TBMQ pods.                                                                                                                                                                                                       | true                                    |
| tbmq.restartPolicy                          | Defines the restart policy for TBMQ pods.                                                                                                                                                                                                                          |                                         |
| **Environment, JVM and Logging Parameters** |                                                                                                                                                                                                                                                                    |                                         |
| tbmq.customEnv                              | Custom environment variables that are always applied, regardless of configuration source.<br/>These variables will be appended to the container environment and will override any conflicting variables set in `existingConfigMap` or `existingJavaOptsConfigMap`. | { SECURITY_MQTT_BASIC_ENABLED: "true" } |
| tbmq.existingConfigMap                      | Name of an existing ConfigMap that will override TBMQ Java and Logback configurations.<br/>If set, this ConfigMap should contain BOTH Java options (`conf` key) and Logback settings (`logback` key).                                                              | ""                                      |
...
| tbmq-ie.statefulSet.annotations             | Custom annotations for StatefulSet.                                                                                                                                                                                                                                | { }                                   |
| **Ports configuration**                     |                                                                                                                                                                                                                                                                    |                                       |
...
| tbmq-ie.annotations                         | Allows users to define custom annotations for the TBMQ IE pods.                                                                                                                                                                                                    | true                                  |                                                                                                                                                                                                                                                                                     |
| tbmq-ie.restartPolicy                       | Defines the restart policy for TBMQ IE pods.                                                                                                                                                                                                                       |                                       |                                                                                                                                                                                                                                                                                     |
| **Environment, JVM and Logging Parameters** |                                                                                                                                                                                                                                                                    |                                       |
| tbmq-ie.customEnv                           | Custom environment variables that are always applied, regardless of configuration source.<br/>These variables will be appended to the container environment and will override any conflicting variables set in `existingConfigMap` or `existingJavaOptsConfigMap`. | { }                                   |
| tbmq-ie.existingConfigMap                   | Name of an existing ConfigMap that will override TBMQ-IE Java and Logback configurations.<br/>If set, this ConfigMap should contain BOTH Java options (`conf` key) and Logback settings (`logback` key).                                                           | ""                                    |
...
| kafka.extraConfig                            | Additional Kafka configuration appended to the default settings.                                                                                                                                                         | auto.create.topics.enable=false<br/>default.replication.factor=2<br/>offsets.topic.replication.factor=3<br/>transaction.state.log.replication.factor=3<br/>transaction.state.log.min.isr=2 |
| **Kafka Listeners**                          |                                                                                                                                                                                                                          |                                                                                                                                                                                            |
...
| redis-cluster.usePassword                                      | Enable authentication for Redis.                                                                                                                                                                                         | true              |
| redis-cluster.password                                         | Redis password (change for production).                                                                                                                                                                                  | "myredispassword" |
| **Pod Disruption Budget (PDB)**                                |                                                                                                                                                                                                                          |                   |
...
| loadbalancer.http.enabled                               | Enables the HTTP Load Balancer (Application Layer - L7).                                                                                                                                                                                                                                                                                                            | false                    |
| loadbalancer.mqtt.enabled                               | Enables the MQTT Load Balancer (Transport Layer - L4).                                                                                                                                                                                                                                                                                                              | false                    |
| **HTTP(S) Configuration**                               |                                                                                                                                                                                                                                                                                                                                                                     |                          |
| loadbalancer.http.ssl.enabled                           | Enables HTTPS termination at the load balancer level.                                                                                                                                                                                                                                                                                                               | false                    |
| loadbalancer.http.ssl.certificateRef                    | SSL certificate reference (depends on loadbalancer type).<br/>AWS: The ACM certificate ARN for ALB.<br/>Azure: The appgw-ssl-certificate value in Application Gateway.<br/>GCP: The name of the ManagedCertificate resource.<br/>Nginx: Not implemented.                                                                                                            | ""                       |
| loadbalancer.http.ssl.domains                           | List of domains for HTTPS traffic.<br/>AWS: Not used in Ingress. Domains are part of the ACM certificate specified in certificateRef.<br/>Azure: Not used in Ingress. Domains are managed directly in Application Gateway.<br/>GCP: Required in Ingress. Domains must be listed for GCP ManagedCertificate to issue an SSL certificate.<br/>Nginx: Not implemented. | ["www.example.com"]      |
...
| loadbalancer.mqtt.mutualTls.enabled                     | Enables two-way TLS (Mutual TLS or mTLS) - Handled at the TBMQ application level. This ensures that both the client and the server authenticate each other. Requires server certificate and private key. TLS Termination (see below) will be ignored if this is enabled.                                                                                            | false                    |
| loadbalancer.mqtt.mutualTls.configMapName               | Name of the ConfigMap containing server certificate and private key. How to create a config map will be described further.                                                                                                                                                                                                                                          | "tbmq-node-mqtts-config" |
| loadbalancer.mqtt.mutualTls.privateKeyPasswordSecret    | Name of the Secret storing the private key password. Required only if your private key is password-protected. How to create a secret will be described further.                                                                                                                                                                                                     | ""                       |
...
| loadbalancer.mqtt.tlsTermination.enabled                | Enables one-way TLS Termination (Handled at Load Balancer Level - L4)<br/>AWS: TLS termination is supported via Network Load Balancer (NLB) with an ACM certificate.<br/>Azure: TLS termination is NOT supported at the Azure Load Balancer level.<br/>GCP: TLS termination is NOT supported at the GCP Network Load Balancer level.<br/>Nginx: Not implemented.    | false                    |
| loadbalancer.mqtt.tlsTermination.certificateRef         | TLS certificate reference for MQTT load balancer.<br/>AWS: The ACM certificate ARN for NLB.<br/>Azure: Not applicable (this option will be ignored).<br/>GCP: Not applicable (this option will be ignored).<br/>Nginx: Not implemented (this option will be ignored).                                                                                               | ""                       |
